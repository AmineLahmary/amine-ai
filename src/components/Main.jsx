...
const Main = () => {
    ...
    const sendBtnClickHandler = async () => {
        ...
        setTimeout(() => answerContainer.current.scrollTo({ top: answerContainer.current.scrollHeight, behavior: "smooth" }), 100)
        ...
        <main className='flex flex-col w-full p-5 h-[95vh]'>
            ...
            <div className='max-w-[768px] w-full m-auto flex flex-col max-h-full'>
                   <div ref={answerContainer} className='answer-container relative flex flex-col gap-8 p-2 pb-4 mb-10 overflow-scroll no-scrollbar rounded-[15px] text-wrap'>
                       ...
                       { loading ? <div className='py-5'><BounceLoader size={30} color="#ffffff" /></div> : null }
                   </div>
                <div className='flex bg-stone-800 px-1 py-1 rounded-full'>
                    <input 
                        type="text"
                        placeholder='Message AmineAI' 
                        className='flex items-center w-full px-5 bg-transparent focus:outline-none' 
                        onChange={e => setInput(e.target.value)}
                        onKeyPress={keyPressHandler} 
                        value={input} />

                    <button onClick={sendBtnClickHandler} className='text-3xl p-2 mr-2 bg-transparent' type='button'>
